...
bool TSP::load(Graph* myGraph) {
	gm = myGraph;
	return true;

	/*
	std::ifstream inputFile;
	inputFile.open("../Data/" + filename);
			...
			if (tmp == "EXPLICIT") {
				...
				for (int i = 0; i < numberOfCities; i++) {
					for (int j = 0; j <= i; j++) {
						...
						gm.setValueOfEdge(i, j, distance);
						gm.setValueOfEdge(j, i, distance);
					}
				}
				delete[] eucCoord;
				return true;
